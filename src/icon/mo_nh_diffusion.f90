...
! DSL stencil injection test
  ...
  USE mo_intp_rbf_rbf_vec_interpol_vertex_dom_a,       ONLY: wrap_run_mo_intp_rbf_rbf_vec_interpol_vertex_dom_a
  USE mo_intp_rbf_rbf_vec_interpol_vertex_dom_b,       ONLY: wrap_run_mo_intp_rbf_rbf_vec_interpol_vertex_dom_b
...
! DSL stencil injection test
    ...
    REAL(wp), DIMENSION(nproma,p_patch%nlev,p_patch%nblks_c) :: u_vert_before
    REAL(wp), DIMENSION(nproma,p_patch%nlev,p_patch%nblks_c) :: v_vert_before
...
!$ACC              u_vert_before, v_vert_before                                                              &
!$ACC              ),                                                                                        &
...
!$ACC PARALLEL IF( i_am_accel_node .AND. acc_on ) DEFAULT(NONE) ASYNC(1)
      u_vert_before(:,:,:) = u_vert(:,:,:)
      v_vert_before(:,:,:) = v_vert(:,:,:)
!$ACC END PARALLEL
      ...
      CALL rbf_vec_interpol_vertex( p_nh_prog%vn, p_patch, p_int,             &
                                    ...
                                    opt_acc_async=.TRUE. )
#endif
      CALL wrap_run_mo_intp_rbf_rbf_vec_interpol_vertex_dom_a(p_e_in=p_nh_prog%vn(:,:,1), &
            ptr_coeff_1=p_int%rbf_vec_coeff_v_1(:,:,1), ptr_coeff_2=p_int%rbf_vec_coeff_v_2(:,:,1), &
            p_u_out=u_vert(:,:,1), p_v_out=v_vert(:,:,1), p_u_out_before=u_vert_before(:,:,1), &
            p_v_out_before=v_vert_before(:,:,1), p_u_out_abs_tol=1e-14_wp, p_v_out_abs_tol=1e-14_wp)
...
!$OMP END PARALLEL
      ...
      CALL wrap_run_mo_nh_diffusion_stencil_01(smag_offset=smag_offset, &
            ...
            kh_smag_ec_abs_tol=1e-18_wp, z_nabla2_e_abs_tol=1e-20_wp)
...
!$ACC END PARALLEL LOOP
#endif
        CALL wrap_run_mo_nh_diffusion_stencil_02(kh_smag_ec=kh_smag_ec(:,:,1), vn=p_nh_prog%vn(:,:,1), &
              e_bln_c_s=p_int%e_bln_c_s(:,:,1), geofac_div=p_int%geofac_div(:,:,1), &
              diff_multfac_smag=diff_multfac_smag(:), kh_c=kh_c(:,:), div=div(:,:), &
              kh_c_before=kh_c_before(:,:), div_before=div_before(:,:))
...
!$ACC END PARALLEL
      ...
      CALL rbf_vec_interpol_vertex( z_nabla2_e, p_patch, p_int, u_vert, v_vert, &
                                    ...
                                    opt_acc_async=.TRUE. )
#endif
      CALL wrap_run_mo_intp_rbf_rbf_vec_interpol_vertex_dom_b(p_e_in=z_nabla2_e(:,:,1), &
            ptr_coeff_1=p_int%rbf_vec_coeff_v_1(:,:,1), ptr_coeff_2=p_int%rbf_vec_coeff_v_2(:,:,1), &
            p_u_out=u_vert(:,:,1), p_v_out=v_vert(:,:,1), p_u_out_before=u_vert_before(:,:,1), &
            p_v_out_before=v_vert_before(:,:,1), p_u_out_abs_tol=1e-21_wp, p_v_out_abs_tol=1e-21_wp)
...
!$ACC END PARALLEL LOOP
        ...
        call wrap_run_mo_nh_diffusion_stencil_05(nudgezone_diff=nudgezone_diff, &
              ...
              vn=p_nh_prog%vn(:,:,1), vn_before=vn_before(:,:,1))
...
!$ACC END PARALLEL LOOP
        ...
        CALL wrap_run_mo_nh_diffusion_stencil_09(diff_multfac_w=diff_multfac_w, &
              ...
              w_before=w_before(:,:,1), w_abs_tol=1e-15_wp)
...
!$OMP END DO
      ...
      call wrap_run_mo_nh_diffusion_stencil_11(thresh_tdiff=thresh_tdiff, &
            ...
            enh_diffu_3d=enh_diffu_3d(:,:,1), enh_diffu_3d_before=enh_diffu_3d_before(:,:,1))
